...
void combinatoria(mpf_t n, mpf_t k){
	mpf_t numerador, denominador,div,combinatoria;
	mpf_init_set(numerador,n);
	mpf_init_set(denominador,k);
	mpf_init_set_ui(div, 1);
	mpf_init_set_ui(combinatoria,1);	
	while(mpf_cmp_ui(denominador,1)>0)
	{
		mpf_div(div,numerador,denominador);
		mpf_mul(combinatoria,combinatoria,div);
		mpf_sub_ui(numerador,numerador,1);
		mpf_sub_ui(denominador,denominador,1);
	}
	mpf_div(div,numerador,denominador);
	mpf_mul(combinatoria,combinatoria,div);
